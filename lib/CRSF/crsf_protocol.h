...
#elif defined(TARGET_RX) || defined(UNIT_TEST)
#ifndef CRSF_RX_MODULE
...
static inline uint16_t ICACHE_RAM_ATTR SWITCH2b_to_CRSF(uint16_t val) 
{ 
    const uint16_t scale = (1795 - 188) / 2;
    const uint16_t result = (val * scale) + 188;
    return result;
    // return round(map(Val, 0, 2, 188, 1795)); 
};
